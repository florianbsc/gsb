    ...
    <div class="container-fluid">
        <div class="col-sm-6 col-12 mb-4 mb-sm-0" style="display: flex">
            ...
            <span class="text-muted text-sm"
                  style="margin-left: 10px; margin-top: 1.5%">Nombre de visites : {{count($visites)}}</span>
                ...
                <table class="table table-hover table-nowrap">
                    ...
                    @foreach($visites as $visite)
                        <tr>
                            ...
                            <td>
                                @if($visite->rapport == '')
                                    <form method="POST" action="{{ route('depot_rapport') }}"
                                          enctype='multipart/form-data'>
                                        @csrf
                                        <label for="rapport" class="btn btn-sm btn-neutral">
                                            <input type="hidden" name="identifiant_employe"
                                                   value="{{ $visite->identifiant_employe }}">
                                            <input type="hidden" name="identifiant_professionnel_de_sante"
                                                   value="{{ $visite->identifiant_professionnel_de_sante }}">
                                            <input type="hidden" name="identifiant_medicament"
                                                   value="{{ $visite->identifiant_medicament }}">
                                            <input type="hidden" name="derniere_visite"
                                                   value="{{ $visite->derniere_visite }}">
